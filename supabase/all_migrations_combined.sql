...
/*
  # Add is_extra and return_picked fields

  1. Changes to budget_items
    - `is_extra` (boolean) — item added in Добор tab after shipment
    - `return_picked` (boolean) — item checked off during return

  2. Changes to warehouse_specification_cables, _connectors, _other
    - `return_picked` (boolean) — item checked off during return
*/

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'budget_items' AND column_name = 'is_extra') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'budget_items' AND column_name = 'return_picked') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'warehouse_specification_cables' AND column_name = 'return_picked') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'warehouse_specification_connectors' AND column_name = 'return_picked') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'warehouse_specification_other' AND column_name = 'return_picked') THEN
...
-- 20260429120000_create_event_types_table.sql
...
/*
  # Reorder warehouse_specification_connectors columns

  Goal: place `connector_item` immediately after `connector_type`.
*/

BEGIN;

CREATE TABLE warehouse_specification_connectors_new (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  event_id uuid NOT NULL REFERENCES events(id) ON DELETE CASCADE,
  connector_type text NOT NULL,
  connector_item text,
...
INSERT INTO warehouse_specification_connectors_new (
  id,
  event_id,
  connector_type,
  connector_item,
...
ALTER TABLE warehouse_specification_connectors_new RENAME TO warehouse_specification_connectors;
